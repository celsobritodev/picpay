...
//    }
    ...
    public void run(String... args) throws Exception {
        ...
        for (WalletType.Enum walletTypeEnum : WalletType.Enum.values()) {
            WalletType walletType = walletTypeEnum.get();
            Long typeId = walletType.getId();
            String typeDescription = walletType.getDescription();
            
            // Verifica explicitamente se já existe
            boolean alreadyExists = walletTypeRepository.existsById(typeId);
            
            if (!alreadyExists ) {
                walletTypeRepository.save(walletType);
                System.out.println("✅ " + typeDescription + " (ID: " + typeId + ") inserido com sucesso");
            }
        }
